...
plantSchema.methods.recordFlowering = function (maleCount = 0, femaleCount = 0) {
  if (!this.flowering.hasStartedFlowering) {
    ...
    this.addTimelineEvent(
      'first_flower',
      `First flower appeared! Male: ${maleCount}, Female: ${femaleCount}`
    );
  }
...
plantSchema.methods.updatePollination = function (pollinationId, updateData) {
  ...
  this.addTimelineEvent('conditions_updated', `${pollination.label} updated`);
...
plantSchema.methods.deletePollination = function (pollinationId) {
  ...
  this.addTimelineEvent('conditions_updated', `${label} deleted`);
...
plantSchema.methods.addFruitDevelopment = function (fruitData) {
  const fruit = {
    ...
    predictedHarvestDate: fruitData.predictedHarvestDate
      ? new Date(fruitData.predictedHarvestDate)
      : null,
...
plantSchema.methods.recordHarvest = function (fruitId, harvestData) {
  ...
  fruit.avgFruitWeightKg = harvestData.fruitCount
    ? harvestData.yieldKg / harvestData.fruitCount
    : null;
